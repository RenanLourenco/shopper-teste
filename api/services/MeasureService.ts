...
export class MeasureService implements IMeasureService {
    ...
    async uploadData(data: UploadDTO): Promise<UploadResponseDTO> {
        const fileName = `${data.customer_code}_${data.measure_type}`
        ...
        const measureValue = await this.gemini.generateContent(fileUri, `return for me only the measure value of this bill, only the measure`)
